...
export const SCREEN_MANIFEST = [
  ...
  workspaceDefinition(screen("HOST-OPS", "/host/analytics", ["/host/analytics", "/host/pricing", "/host/promotions", "/host/exports", "/host/reviews", "/host/badges", "/host/settings", "/host/verification", "/host/properties/archived"], (path) => ({ name: "host-spec", view: path.split("/").at(-1) === "archived" ? "archived" : path.split("/").at(-1) ?? "analytics" }), { title: "Host operations", productArea: "Host", componentKey: "host-spec", roleAccess: ["Host"], navigation: { label: "Host tools", description: "Pricing, reviews, badges, and reports" } })),
