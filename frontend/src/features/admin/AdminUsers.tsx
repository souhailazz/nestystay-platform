...
export function AdminUsers({ token }: AdminUsersProps) {
  ...
  return (
    <div className="page-container container py-6" data-testid="adm-02-page" id="ADM-02">
      ...
      {selectedHost && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="host-verification-review-title"><div className="modal-card w-[min(100%-2rem,560px)]"><div className="flex items-start justify-between gap-3"><div><h3 id="host-verification-review-title">Review {selectedHost.displayName}</h3><p className="subtext">The decision is persisted against this host and visible in their verification status.</p></div><ShieldCheck className="text-deep-hover" /></div><div className="my-4 grid gap-2 rounded-field border border-sand-border bg-shell p-3 text-sm"><span><strong>Email:</strong> {selectedHost.email}</span><span><strong>Document:</strong> {selectedHost.documentType ?? "Not submitted"}</span><span><strong>Current status:</strong> <span className={`badge ${statusClass(selectedHost.status)}`}>{selectedHost.status}</span></span>{selectedHost.submittedAt && <span><strong>Submitted:</strong> {new Date(selectedHost.submittedAt).toLocaleString()}</span>}</div><div className="mb-3 flex flex-wrap gap-2"><button className={`btn btn-sm ${decision === "Approved" ? "btn-primary" : "btn-outline"}`} onClick={() => setDecision("Approved")} type="button"><Check size={14} /> Approve</button><button className={`btn btn-sm ${decision === "Rejected" ? "btn-primary" : "btn-outline"}`} onClick={() => setDecision("Rejected")} type="button"><X size={14} /> Reject</button></div><label className="field-label" htmlFor="host-verification-decision-reason">Decision reason {decision === "Rejected" ? "(required)" : "(optional)"}</label><textarea id="host-verification-decision-reason" className="input-control mt-1" rows={4} placeholder={decision === "Rejected" ? "Explain what the host needs to correct." : "Optional reviewer note"} value={reason} onChange={(event) => setReason(event.target.value)} /><div className="mt-4 flex flex-wrap justify-end gap-2"><button className="btn btn-ghost" onClick={() => setSelectedHost(null)} type="button">Cancel</button><button className="btn btn-primary" onClick={() => void reviewHost(selectedHost, decision, reason)} type="button">Save decision</button></div></div></div>}
    </div>
  );
}
