...
export type InsurancePlan = { code: string; market: string; monthlyAmount: number; currency: string; propertyDamageCoverage: number; accidentalMedicalCoverage: number; coverageSummary: string };
export type InsurancePolicy = { id: string; propertyId: string; hostUserId: string; provider: string; planCode: string; market: string; monthlyAmount: number; currency: string; propertyDamageCoverage: number; accidentalMedicalCoverage: number; status: string; providerReference?: string | null; effectiveAt?: string | null; renewsAt?: string | null; cancelledAt?: string | null; failureReason?: string | null };
export type InsuranceProperty = { propertyId: string; propertyTitle: string; coverageFlag: boolean; status: string; policy?: InsurancePolicy | null };
export type ManagedInsurance = { propertyId: string; propertyTitle: string; hostUserId: string; status: string; planCode?: string | null; provider?: string | null; providerReference?: string | null; renewsAt?: string | null; failureReason?: string | null };
export type InsuranceClaim = { id: string; policyId: string; propertyId: string; bookingId?: string | null; incidentAt: string; description: string; evidenceJson: string; status: string; providerReference?: string | null; submittedAt: string };
export type InsurancePolicyEvent = { id: string; policyId: string; propertyId: string; fromStatus: string; toStatus: string; reason: string; idempotencyKey?: string | null; createdAt: string };
...
export const api = {
  ...
  getInsurancePlans: () => request<InsurancePlan[]>("/insurance/plans"),
  getInsuranceProperty: (propertyId: string, token: string) => request<InsuranceProperty>(`/insurance/properties/${propertyId}`, { token }),
  getInsuranceClaims: (propertyId: string, token: string) => request<InsuranceClaim[]>(`/insurance/properties/${propertyId}/claims`, { token }),
  getInsurancePolicyEvents: (propertyId: string, token: string) => request<InsurancePolicyEvent[]>(`/insurance/properties/${propertyId}/policy/events`, { token }),
  activateInsurance: (propertyId: string, token: string, body: { planCode: string; idempotencyKey: string }) => request<InsurancePolicy>(`/insurance/properties/${propertyId}/policy`, { method: "POST", token, body }),
  cancelInsurance: (propertyId: string, token: string, idempotencyKey: string) => request<InsurancePolicy>(`/insurance/properties/${propertyId}/policy/cancel`, { method: "POST", token, body: { idempotencyKey } }),
  renewInsurance: (propertyId: string, token: string, idempotencyKey: string) => request<InsurancePolicy>(`/insurance/properties/${propertyId}/policy/renew`, { method: "POST", token, body: { idempotencyKey } }),
  markInsuranceRenewalDue: (propertyId: string, token: string) => request<InsurancePolicy>(`/insurance/properties/${propertyId}/policy/renewal-due`, { method: "POST", token }),
  submitInsuranceClaim: (propertyId: string, token: string, body: { incidentAt: string; description: string; bookingId?: string; evidenceJson?: string }) => request<InsuranceClaim>(`/insurance/properties/${propertyId}/claims`, { method: "POST", token, body }),
  getManagedInsurance: (token: string) => request<ManagedInsurance[]>("/insurance/managed", { token }),
  getInsurancePolicies: (token: string) => request<InsurancePolicy[]>("/insurance/policies", { token }),
