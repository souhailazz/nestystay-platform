...
async function seed(api) {
  ...
  for (const level of ["Verified", "Trusted", "Wellness"]) {
    try { await jsonRequest(api, "post", "/api/badges-pricing/badges/purchase-intent", { subjectType: "Host", subjectId: sessions.host.userId, level, hostVerificationPassed: true, completedApprovedBookings: 3, hasPropertyAddress: true, hasWellnessSubscription: true, idempotencyKey: `client-demo-${level.toLowerCase()}-${sessions.host.userId}` }, adminToken); } catch { /* fixture may already exist */ }
  }
