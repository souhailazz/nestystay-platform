...
test("PM cookie login, invoice loading, utility persistence and owner portal", async ({ page, request }, testInfo) => {
  ...
  const documentCard = page.locator("section").filter({ has: page.getByRole("heading", { name: "Document storage", exact: true }) }).first();
  await documentCard.getByLabel("Document title", { exact: true }).fill("Browser compliance document");
  await documentCard.getByLabel("Expiry date (optional)", { exact: true }).fill("2030-01-01");
  await documentCard.getByLabel("File", { exact: true }).setInputFiles({ name: "compliance.pdf", mimeType: "application/pdf", buffer: Buffer.from("%PDF-1.7\nNestyStay browser document\n") });
  const storedDocument = page.waitForResponse(r => r.url().endsWith("/api/property-manager/documents") && r.request().method() === "POST");
  await documentCard.getByRole("button", { name: "Store document", exact: true }).click();
  expect((await storedDocument).ok()).toBeTruthy();
  await expect(documentCard.getByText("expires 2030-01-01", { exact: false })).toBeVisible();
  const exportResponse = page.waitForResponse(r => r.url().endsWith("/api/property-manager/documents/exports") && r.request().method() === "POST");
  await page.getByRole("button", { name: "Export all as ZIP", exact: true }).click();
  expect((await exportResponse).ok()).toBeTruthy();
  await expect(page.getByText(/Document ZIP export is ready|Export is still processing/, { exact: false })).toBeVisible({ timeout: 25000 });
  await page.goto("/pm/subscription");
  await expect(page.getByRole("heading", { name: "Subscription lifecycle", exact: true })).toBeVisible();
  await expect(page.getByText("Current plan", { exact: true })).toBeVisible();
  await expect(page.getByText("Provider: LOCAL_TEST_READY", { exact: true })).toBeVisible();
  ...
  const pauseResponse = page.waitForResponse(r => r.url().endsWith("/api/property-manager/subscription/change") && r.request().method() === "POST");
  await page.getByRole("button", { name: "Apply change", exact: true }).click();
  expect((await pauseResponse).ok()).toBeTruthy();
  await expect(page.getByText("PAUSED", { exact: true }).first()).toBeVisible();
  const resumeResponse = page.waitForResponse(r => r.url().endsWith("/api/property-manager/subscription/change") && r.request().method() === "POST");
  await pauseAction.selectOption("RESUME");
  await page.getByRole("button", { name: "Apply change", exact: true }).click();
  expect((await resumeResponse).ok()).toBeTruthy();
  await expect(page.getByText("ACTIVE", { exact: true }).first()).toBeVisible();
  const autoRenewResponse = page.waitForResponse(r => r.url().endsWith("/api/property-manager/subscription/change") && r.request().method() === "POST");
