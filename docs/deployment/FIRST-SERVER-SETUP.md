...
2. Point the domain's web A/AAAA record to the VPS through Cloudflare (free DNS/WAF). Keep database, Redis, private storage and monitoring ports private.
...
4. Set strong unique PostgreSQL, Redis, local-storage signing and application secrets. Set Stripe, the approved identity-provider and Brevo values only in the secret manager.
