...
| Monitoring | Prometheus/Grafana + Loki + Uptime Kuma | Self-hosted | VPS disk | Yes before go-live | PASS (Compose observability profile and starter alerts/dashboards) | PASS (config validation); delivery/incident drill requires deployment | Health endpoints/logs |
