...
| Mobile PM parity | YES | Representative responsive routes | Complete for tested routes | Complete | Complete | **Not fully certified at every requested width/module** | PARTIAL — exact | Run focused 320/360/390/414/430 checks for every PM table, modal, calendar, finance and staff screen |
...
- Backend focused authorization suite: **3 passed, 0 failed**.
...
- Existing full backend baseline was **193 passed, 0 failed, 1 legitimate MinIO skip**; after adding this focused suite the full rerun is **196 passed, 0 failed, 1 skipped**.
- Existing browser baseline remains **224 scheduled, 188 passed, 0 failed, 36 explicit skips, 0 not-run**. The browser run is not a substitute for the direct API matrix.
...
- **Still partial:** universal mobile PM parity and the literal every-resource/every-role IDOR matrix.
...
- **SOUHAIL:** add and execute the remaining dedicated cross-account fixtures for messages/attachments, Wellness assignments/reports, provider private records, and Admin-only resources; complete the full PM mobile width/module matrix.
