...
| 12 | M1 eKYC | FULL local/test; live provider blocked | Camera/media capture, file fallback, preview/compression/retry and secure upload are implemented; live Stripe Identity validation is not possible without client credentials, return URL and signed webhook configuration. Alibaba is not an active runtime provider. |
...
- Live Stripe/Stripe Identity, external email/SMS/push, full cross-browser role matrix, formal manual WCAG sign-off and production object-storage read/retention certification remain separate provider/deployment gates. Alibaba is not an active runtime provider; historical migration snapshots are retained for schema history only.
