...
The frontend branch fixes role-aware post-login routing. The previous deployed build routed every successful password login through `/guest-dashboard` because it read React auth state before the asynchronous state update completed. The current branch reads the synchronously persisted session, centralizes passwordless routing, sends Admin users to `/admin`, rejects unsafe external `returnTo` targets, and clears revoked/forbidden cached sessions. These behaviors are covered by `postAuthRoute` and `useAuth` tests. The latest commit `5bd5270eefe81ef5a1c5726b0e74b3bb42c9e7ee` is pushed to `codex/m1-m2-runtime-hardening`; staging must redeploy it before the browser behavior can be rechecked.
