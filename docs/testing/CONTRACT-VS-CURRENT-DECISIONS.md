...
| InsuraGuest | Local deterministic provider seam, exact plan catalog, host policy lifecycle, persisted transition history, 72-hour claims intake, scoped property authorization and PM/admin visibility | COMPLETE LOCALLY / EXTERNAL PROVIDER REQUIRED FOR LIVE ACCEPTANCE | The contract describes an optional host insurance add-on with $50/$69/$99 plans and no damage deposit/waiver for covered properties. Local code now exercises the full application lifecycle without claiming a live InsuraGuest approval or external insurer rail. |
