...
The 11 skips are conditional provider/privileged checks requiring an injected admin token, explicit MinIO browser mode, or non-local smoke credentials. They are recorded as skips by the test suite; they are not counted as passes.
