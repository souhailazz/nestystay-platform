...
| M5 Property Manager | Multi-owner portfolio, owner portal, finance, maintenance, utilities, governance, documents, gate and subscription | P0 money/authority foundation is implemented; release-candidate operational hardening is additive and partially complete | `/pm/*`, `/owner/dashboard`, `/gate` call real APIs with cookie sessions; professional operations now includes linked-listing protection, reservation safety/history, quote/approval enforcement, readiness guards and live dashboard KPIs | 150 backend tests, 37 Vitest, dedicated M5 browser journeys pass; fresh 34-area audit identifies remaining launch blockers in reservations, maintenance/readiness, utilities, documents, governance and staff scope | PARTIAL — launch blockers remain; see `M5-RELEASE-CANDIDATE-AUDIT.md` |
...
- Honest boundaries: coverage coordinates, server-rendered report PDFs, payout statements and dispute state are now available locally. Live provider payment rails, external bank verification, SMS/push delivery and production reminder/expiry workers remain separate certification/deployment gates; the local subscription scheduler and document expiry/export workers are implemented and tested.
- Release security baseline: backend build is warning-free and the transitive NuGet vulnerability audit is clean after pinning `Microsoft.Bcl.Memory 10.0.12` for the WebAuthn dependency.
