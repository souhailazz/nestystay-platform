...
| M5 Property Manager | Multi-owner portfolio, owner portal, finance, maintenance, utilities, governance, documents, gate and subscription | Implemented with manager/owner scope, idempotency, reviewed FK relationships and scheduled subscription worker | `/pm/*`, `/owner/dashboard`, `/gate` call real API with HttpOnly cookie sessions and CSRF protection; notices support scheduling, audience targeting and acknowledgement deadlines; subscription route exposes plan limits, billing history, retry, pause/resume, auto-renew, scheduled downgrade and reactivation | 88 API tests, 35 Vitest, configured Playwright matrix 91 passed / 52 intentional credential/viewport skips; PM subscription/document/utility journey also passed tablet/mobile; Firefox/WebKit smoke passed | PASS locally |
