...
{
    [Fact]
    public async Task GenericWebhookRejectsUnsupportedProviderInProduction()
    {
    ...
    [Fact]
    public async Task GenericWebhookRejectsUnsupportedProviderEvenWithMatchingSecret()
    {
        var controller = CreateController();
    ...
    [Fact]
    public async Task StripeRawWebhookMapsProcessingPaymentIntentToPending()
    {
        var eventId = $"evt_{Guid.NewGuid():N}";
        var payload = $$"""
            {
        ...
        Assert.IsType<AcceptedResult>(result);
        var update = Assert.Single(store.PaymentWebhookUpdates);
        Assert.Equal("payment_intent.processing", update.EventType);
        Assert.Equal("pi_test_processing", update.PaymentIntentReference);
        Assert.Equal(PaymentStatus.Pending, update.Status);
    }

    [Fact]
    public async Task StripeRawWebhookPreservesPaymentFailureReason()
    {
        var eventId = $"evt_{Guid.NewGuid():N}";
        var payload = $$"""
            {
        ...
        Assert.IsType<AcceptedResult>(result);
        var update = Assert.Single(store.PaymentWebhookUpdates);
        Assert.Equal(PaymentStatus.Failed, update.Status);
        Assert.Equal("Your card was declined.", update.Reason);
    }

    [Fact]
    public async Task StripeRawWebhookAppliesOnlySucceededRefundEvents()
    {
        var store = new StubPhaseOneStore();
        var eventStore = new StubProviderEventStore();

        var pendingEventId = $"evt_refund_pending_{Guid.NewGuid():N}";
        var pendingPayload = $$"""
            {
              "id": "{{pendingEventId}}",
              "type": "refund.created",
            ...
            """;
        var pendingController = CreateController(store, eventStore);
        pendingController.Request.Headers["Stripe-Signature"] = CreateStripeSignature(pendingPayload);
        SetRawBody(pendingController, pendingPayload);

        await pendingController.ReceiveStripeRaw(CancellationToken.None);

        Assert.Empty(store.PaymentWebhookUpdates);

        var succeededEventId = $"evt_refund_succeeded_{Guid.NewGuid():N}";
        var succeededPayload = pendingPayload
            .Replace(pendingEventId, succeededEventId, StringComparison.Ordinal)
            .Replace("re_pending", "re_succeeded", StringComparison.Ordinal)
            .Replace("pending", "succeeded", StringComparison.Ordinal);
        var succeededController = CreateController(store, eventStore);
        succeededController.Request.Headers["Stripe-Signature"] = CreateStripeSignature(succeededPayload);
        SetRawBody(succeededController, succeededPayload);

        await succeededController.ReceiveStripeRaw(CancellationToken.None);

        var update = Assert.Single(store.PaymentWebhookUpdates);
        Assert.Equal("refund.created", update.EventType);
        Assert.Equal("pi_refund_test", update.PaymentIntentReference);
        Assert.Equal("re_succeeded", update.ProviderReference);
        Assert.Equal(PaymentStatus.Refunded, update.Status);
        Assert.Equal(610.50m, update.Amount);
    }

    [Fact]
    public async Task StripeRawWebhookResolvesIdentityVerificationEvents()
    {
        var eventId = $"evt_{Guid.NewGuid():N}";
        ...
        var payload = $$"""
            {
              ...
              "data": {
                "object": {
                  "id": "vs_test_identity",
                  "client_reference_id": "{{bookingId:N}}",
                  "metadata": { "booking_id": "{{bookingId:N}}" },
                  "status": "verified",
                  "verified_outputs": { "first_name": "Private", "last_name": "Identity" }
                }
              }
            }
            """;
        ...
        Assert.IsType<AcceptedResult>(result);
        var resolution = Assert.Single(store.VerificationResolutions);
        Assert.True(resolution.Passed);
        Assert.Equal("vs_test_identity", resolution.ProviderReference);
        var stored = Assert.Single(eventStore.Records);
        Assert.Equal(ProviderKind.Ekyc, stored.Kind);
        Assert.Equal("Stripe Identity", stored.ProviderName);
        Assert.Equal("identity.verification_session.verified", stored.EventType);
        Assert.DoesNotContain("verified_outputs", stored.PayloadJson, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("Private", stored.PayloadJson, StringComparison.OrdinalIgnoreCase);
        Assert.Equal("Processed", Assert.Single(eventStore.Results).Status);
    }
    ...
    private sealed class StubPhaseOneStore : IPhaseOneStore
    {
        ...
        public BookingDto? GetBooking(Guid id) => null;
        ...
        public Task<BookingDto?> ResolveVerificationAsync(Guid bookingId, ResolveVerificationRequest request, CancellationToken cancellationToken) =>
            RecordVerificationResolution(request);

        public List<ResolveVerificationRequest> VerificationResolutions { get; } = [];

        private Task<BookingDto?> RecordVerificationResolution(ResolveVerificationRequest request)
        {
            VerificationResolutions.Add(request);
