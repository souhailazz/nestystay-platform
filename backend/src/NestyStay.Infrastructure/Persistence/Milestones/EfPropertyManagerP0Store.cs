    ...
    TimeProvider timeProvider) : IPropertyManagerP0Store
{
    ...
    private static PropertyDto ToDto(MilestoneManagerProperty item) => new(item.Id, item.OwnerUserId, item.CommunityId, item.Title, item.UnitNumber, item.Address, item.Status, item.OccupancyStatus);
