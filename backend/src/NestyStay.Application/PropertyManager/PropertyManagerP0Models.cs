...
public sealed record P0CreateApprovalRequest(Guid OwnerUserId, Guid? PropertyId, Guid? AgreementId, Guid? FeeRuleId, string ApprovalType, string Description, decimal Amount, string Currency, IReadOnlyList<Guid>? EvidenceDocumentIds = null);
...
public sealed record P0ApprovalDecisionRequest(string Status, string Reason, long RowVersion = 1);
