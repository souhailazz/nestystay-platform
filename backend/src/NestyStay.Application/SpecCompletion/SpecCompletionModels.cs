...
public interface ISpecCompletionStore
{
    ...
    Task<IReadOnlyList<DirectoryProviderDocumentDto>> GetDirectoryProviderDocumentsAsync(Guid providerId, Guid actorUserId, CancellationToken cancellationToken);
    Task<DirectoryProviderDocumentUploadDto> PrepareDirectoryProviderDocumentUploadAsync(Guid providerId, Guid actorUserId, PrepareDirectoryProviderDocumentUploadRequest request, CancellationToken cancellationToken);
    Task<DirectoryProviderDocumentUploadDto> UploadDirectoryProviderDocumentContentAsync(Guid providerId, Guid actorUserId, Guid documentId, string contentType, long sizeBytes, Stream content, CancellationToken cancellationToken);
    Task<DirectoryProviderDocumentDownloadDto> GetDirectoryProviderDocumentDownloadAsync(Guid providerId, Guid actorUserId, Guid documentId, CancellationToken cancellationToken);
...
public sealed record DirectoryProviderDocumentDownloadDto(Guid Id, string FileName, string ContentType, long SizeBytes, string Url, DateTimeOffset ExpiresAt);
