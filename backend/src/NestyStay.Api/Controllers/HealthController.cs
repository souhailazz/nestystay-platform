    ...
    NestyStayDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> GetIntegrations(CancellationToken cancellationToken)
    {
        ...
        var stripeIdentityConfigured = HasSetting("Integrations:StripeSecretKey", "STRIPE_SECRET_KEY") &&
                                       HasSetting("Integrations:StripeIdentityReturnUrl", "STRIPE_IDENTITY_RETURN_URL");
        ...
        return Ok(new
        {
            ...
            services = new[]
            {
                ...
                Service("identity", ekycProvider.ProviderName, stripeIdentityConfigured ? "CONFIGURED" : "BLOCKED_EXTERNAL", "Stripe Identity adapter"),
                ...
                Service("storage", storageProvider.ProviderName, "CONFIGURED", "Private server-local storage; objects are outside the web root and served only through authorized or signed API paths"),
